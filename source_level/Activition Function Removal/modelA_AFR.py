...
def getModel(k):
    ...
    model = Sequential()
    # Keras no activation == Linear Activation
    # activation='relu', 模型 AFR0
    model.add(Conv2D(filters=6, kernel_size=(5, 5),padding='valid',activation='relu',input_shape=(28, 28, 1)))
    model.add(MaxPooling2D(pool_size=(2, 2)))
    #, activation='relu', 模型 AFR1
    model.add(Conv2D(filters=16, kernel_size=(5, 5),padding='valid', activation='relu'))
    model.add(MaxPooling2D(pool_size=(2, 2)))
    model.add(Flatten())
    # , activation='relu' 模型 AFR2
    model.add(Dense(120, activation='relu'))
    # , activation='relu'  模型 AFR3
    model.add(Dense(84, activation='relu'))
    # activation='softmax'  模型 AFR4

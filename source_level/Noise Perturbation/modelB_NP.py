...
def getModel(k):
    ...
    print(randomIndex)
    # 噪声扰动
    with open("modelB\B" + str(k) + "\\" + "randomResult_" + str(k) + ".txt", "a+", encoding="utf-8") as f:
        for i in range(600):
    ...
    # 写入文件中
    with open("modelB\B" + str(k) + "\\" + "np_" + str(k) + ".txt", "a+", encoding="utf-8") as f:
        for i in range(600):
    ...
    # 噪声扰动
    # 就是随机取一个中心点，实现3*3的随机取值(60000*28*28)
    ...
    for i in range(600):
        ...
        with open("modelB\B" + str(k) + "\\" + "npArray_" + str(k) + ".txt", "a+", encoding="utf-8") as arrayF:
        ...
        with open("modelB\B" + str(k) + "\\" + "initialArray_" + str(k) + ".txt", "a+", encoding="utf-8") as initialF:
        ...
        with open("modelB\B" + str(k) + "\\" + "npedArray_" + str(k) + ".txt", "a+", encoding="utf-8") as npedF:
    ...
    with open('modelB\\'+'ModelBResults.txt',"a+",encoding="utf-8") as result_f :
        result_f.write('Test Loss: %.4f' % score[0])
        result_f.write("\t")
        result_f.write('Test accuracy: %.4f'% score[1])
        result_f.write("\n")
        result_f.close()
    model.save('modelB\\'+'ModelB_NP'+str(k)+'.hdf5')
